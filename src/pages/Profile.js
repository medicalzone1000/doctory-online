...
if (user) {
  ...
  $('#bio').value = user.bio || user.profile?.bio || '';
  $('#profile-email-input').value = user.email || '';
  $('#profile-name').textContent = authService.getDisplayName(user);
  $('#profile-email').textContent = user.email || '—';
  $('#profile-role').textContent = user.role || user.profile?.role || 'user';

  const initials = authService.getDisplayName(user)
    .split(' ')
    .map(n => n[0])
    .join('')
    .toUpperCase()
...
passwordForm.addEventListener('submit', async (e) => {
  ...
  try {
    await authApi.changePassword({ newPassword: data.newPassword });
