      ...
      <tr>
        <td>
          <div style="display:flex;align-items:center;gap:var(--space-3)">
            ...
            <span style="font-weight:var(--font-weight-medium);color:var(--color-text-primary)">${u.name || '—'}</span>
          </div>
        </td>
        <td>${u.email || '—'}</td>
        ...
        <td>
          <div class="admin-table__actions">
            <button class="btn btn--ghost btn--xs" data-role-id="${u._id || u.id}" data-role-name="${u.name}" data-current-role="${u.role || 'user'}">Role</button>
            <button class="btn btn--ghost btn--xs" data-toggle-id="${u._id || u.id}" data-active="${u.active !== false}">
              ${u.active === false ? 'Activate' : 'Deactivate'}
            </button>
            <button class="btn btn--danger btn--xs" data-delete-id="${u._id || u.id}" data-delete-name="${u.name}">Delete</button>
          </div>
        </td>
      </tr>
    `).join('');
