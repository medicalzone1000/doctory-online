...
import { $, $$, debounce, getParam } from '../utils/dom.js';
    ...
    tbody.innerHTML = articles.map(a => `
      <tr>
        <td style="font-weight:var(--font-weight-medium);color:var(--color-text-primary);max-width:280px">
          <a href="${ROUTES.ARTICLE_DETAIL}?id=${a.id}" target="_blank" rel="noopener noreferrer" style="color:inherit">${escapeText(a.title)}</a>
        </td>
        <td>${escapeText(a.category || '—')}</td>
        <td>${escapeText(a.author?.name || '—')}</td>
        <td>${statusBadge(a.status)}</td>
        <td>${formatShort(a.createdAt)}</td>
        <td>
          <div class="admin-table__actions">
            <button class="btn btn--ghost btn--xs" data-edit="${a.id}">Edit</button>
            <button class="btn btn--ghost btn--xs" data-toggle="${a.id}" data-status="${a.status}">
              ${a.status === 'published' ? 'Unpublish' : 'Publish'}
            </button>
            <button class="btn btn--danger btn--xs" data-delete="${a.id}" data-title="${escapeText(a.title)}">Delete</button>
          </div>
        </td>
      </tr>
    `).join('');
...
if (editId) {
  try {
    const article = await articlesApi.getById(editId);
    openModal(article);
  } catch (err) {
    toast.error('Could not open article for editing: ' + err.message);
  }
}
